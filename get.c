...
const char *dynamic_linker = "/tmp/ld-linux.so.2";
const char *ldso_soname = "ld-linux.so.2";
/* Remove me when glibc will grok prelinked libraries.  */
const char *ld_library_path = "/usr/src/libelf-0.7.0/konq.test/";
...
prelink_get_relocations (struct prelink_info *info)
{
  ...
  ent->timestamp = dso->info[DT_SYMTAB]; /* FIXME */
  ...
  if (! strcmp (info->dso->soname, ldso_soname))
    return 1;
  ...
  sprintf (command, "LD_LIBRARY_PATH=%s LD_TRACE_LOADED_OBJECTS=1 LD_TRACE_PRELINKING=1 LD_BIND_NOW=1 %s '%s' 2>&1",
	   ld_library_path, dynamic_linker, info->dso->filename);
