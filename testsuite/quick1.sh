...
BINS="$BINS quick1.tree/usr/bin/bin3"
savelibs
...
readelf -a quick1.tree/usr/bin/bin3 >> quick1.log 2>&1 || exit 7
BINS="quick1.tree/usr/bin/bin1 quick1.tree/usr/bin/bin3"; LIBS=
comparelibs >> quick1.log 2>&1 || exit 8
...
cp -p quick1.tree/usr/lib/lib2{.later,}.so.orig
mv -f quick1.tree/usr/lib/lib7.so{,.old}
mv -f quick1.tree/usr/lib/lib7.so{,.old}.orig
cp -p quick1.tree/usr/bin/bin3{,.old}
chmod 644 quick1.tree/usr/bin/bin3.old
cp -p quick1.tree/usr/lib/lib7{.later,}.so
cp -p quick1.tree/usr/lib/lib7{.later,}.so.orig
echo $PRELINK ${PRELINK_OPTS--vm} -aqvvvvv >> quick1.log
$PRELINK ${PRELINK_OPTS--vm} -aqvvvvv > quick1.tree/etc/log5 2>&1 || exit 9
cat quick1.tree/etc/log5 >> quick1.log
LD_LIBRARY_PATH=quick1.tree/lib:quick1.tree/usr/lib quick1.tree/usr/bin/bin1 || exit 10
readelf -a quick1.tree/usr/bin/bin1 >> quick1.log 2>&1 || exit 11
readelf -a quick1.tree/usr/bin/bin3 >> quick1.log 2>&1 || exit 12
grep -q ^`echo $PRELINK | sed 's/ .*$/: /'` quick1.log && exit 13
...
CHECKE="$B1 $B2 $B3"; CHECKL="$SL $L1 $L2 $L4 $L5 $L6 $L7" PREL="$CHECKE $CHECKL"; ASSUME=""
check_log quick1.tree/etc/log1
CHECKE=""; CHECKL=""; PREL=""; ASSUME="$B1 $B2 $B3 $SL $L1 $L2 $L4 $L5 $L6 $L7"
check_log quick1.tree/etc/log2
CHECKE="$B1 $B3"; CHECKL="$SL $L1 $L2 $L5 $L6 $L7"; PREL=""; ASSUME="$B2 $L4"
check_log quick1.tree/etc/log3
CHECKE=""; CHECKL=""; PREL=""; ASSUME="$B1 $B2 $B3 $SL $L1 $L2 $L4 $L5 $L6 $L7"
check_log quick1.tree/etc/log4
CHECKE="$B1 $B3"; CHECKL="$SL $L1 $L2 $L3 $L7"; PREL="$B1 $B3 $L2 $L3 $L7"; ASSUME="$B2 $L4"
check_log quick1.tree/etc/log5
BINS="$B1 $B2 $B3"
LIBS="$SL $L1 $L2 $L3 $L4 $L5 $L6 $L7 $L2.old $L7.old"
comparelibs >> quick1.log 2>&1 || exit 14
...
$PRELINK -ua >> quick1.log 2>&1 || exit 15
for i in $BINS $SL $L1 $L2 $L3 $L4 $L5 $L6 $L7; do
  cmp -s $i.orig $i || exit 16
  mv -f $i.prelinked $i
done
chmod 755 $BINS
exit 0
